...
class JsonInputReader(BaseInputReader):
    def __init__(self, types_path: str, tokenizer: BertTokenizer, neg_entity_count: int = None,
                 ...
                 logger: Logger = None):
        super().__init__(types_path, tokenizer, neg_entity_count, neg_rel_count,
                         max_span_size, logger)
    ...
    def _parse_entities(self, jentities, doc_tokens, dataset) -> List[Entity]:
        ...
        for entity_idx, jentity in enumerate(jentities):
            ...
            # We may have a special case if some entities were included because of BERT's length restrictions
    ...
    def _parse_relations(self, jrelations, entities, dataset) -> List[Relation]:
        ...
        for jrelation in jrelations:
            ...
            # Samples may be longer than the length restriction of BERT allows
            # We test if some relations were cut-off because of this.
            # If so, we exclude the relations
            if max([max(v) for (k,v) in args.items()]) >= len(entities):
                continue

            head_key, tail_key = sorted(args)
            head_idx = args[head_key][0]
            tail_idx = args[tail_key][0]

            # create relation
            # args = [entities[arg_idx] for arg_idx in args]

            relation = dataset.create_relation(relation_type, entities[head_idx], entities[tail_idx])

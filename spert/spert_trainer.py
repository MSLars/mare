...
class SpERTTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        relation_ner_macrio = -1.0

        # eval validation set
        if args.init_eval:
            relation_ner_macrio = self._eval(model, validation_dataset, input_reader, 0, updates_epoch)
        ...
        for epoch in range(args.epochs):
            ...
            if not args.final_eval or (epoch == args.epochs - 1):
                relation_ner_macrio = self._eval(model, validation_dataset, input_reader, epoch + 1, updates_epoch)
    ...
    def _eval(self, model: torch.nn.Module, dataset: Dataset, input_reader: JsonInputReader,
        ...
        relation_macro_ner = self._log_eval(*ner_eval, *rel_eval, *rel_nec_eval,
